...
class GAN:
    def __init__(self):
        ...
        self.combined.compile(loss=['binary_crossentropy','categorical_crossentropy'], optimizer='adam')
    ...
    def build_discriminator(self):
        ...
        dis = Model(inputs=i_input, outputs=model)
    ...
    def load_mnist(self):
        ...
        m_in = Input((28,28,1),name='mnist_input')
        m = m(m_in)
        m = Reshape((self.l_dim,),name='cate')(m)
        mnist = Model(inputs=m_in,outputs=m)
        mnist.summary()
        return mnist
    ...
    def build_combined(self):
        ...
        self.discriminator.trainable = False
        valid = self.discriminator(img)
        self.mnist.trainable = False
        cate = self.mnist(img)
        model = Model(inputs=[z,l_g],outputs=[valid,cate])
    ...
    def train(self, iterations, batch_size=128, save_interval=50, model_interval=1000, check_noise=None, check_label=None, raw=5, col=5):
        ...
        for iteration in range(iterations):
            ...
            g_loss = self.combined.train_on_batch([noise,gen_lbls], [np.ones((batch_size, 1)),gen_lbls])

            print("%d [D loss: %f, acc.: %.2f%%] [G loss: %f,%f]" % (iteration, d_loss[0], 100 * d_loss[1], g_loss[0],g_loss[1]))
